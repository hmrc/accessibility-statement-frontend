...
If you are a service that requires a custom template like VOA or C-HGV, please include the property `templateOverride: ` and set the value
to either `VOA` or `C-HGV`. If you are not one of those services you do not need to include this property as the template will always default to the `HMRC` template. 
